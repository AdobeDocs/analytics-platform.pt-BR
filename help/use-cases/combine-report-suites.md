...
## Como as diferenças de esquema entre os conjuntos de relatórios são problemáticas
...
Por uma questão de simplicidade, diremos que essas são as únicas eVars definidas para ambos os conjuntos de relatórios.
...
Esta situação resultará em relatórios sem sentido para o eVar1 e o eVar2:

- Os campos de eVar conterão uma mistura de valores com significados semânticos diferentes.
- Os termos de pesquisa serão distribuídos entre o eVar1 e o eVar2.
- Não será possível usar modelos de atribuição diferentes para cada um dos termos de pesquisa, unidades de negócios e categorias de clientes.

## Uso da Preparação de dados da AEP para resolver diferenças de esquema entre conjuntos de relatórios para uso no CJA

A funcionalidade de Preparação de dados do AEP é integrada ao Conector de fonte do Analytics e pode ser usada para resolver as diferenças de esquema descritas no cenário acima, resultando em eVars com significados consistentes na visualização de dados do CJA. Segue-se uma descrição de como isso pode ser feito. As convenções de nomenclatura usadas abaixo podem ser personalizadas para atender às suas necessidades.

Antes de criar os fluxos de dados da conexão de origem para o Conjunto de relatórios A e o Conjunto de relatórios B, crie um grupo de campos personalizado no AEP (vamos chamá-lo **Campos unificados** no nosso exemplo) que contém os seguintes campos:
...
Crie um novo schema no AEP (vamos chamá-lo **Esquema unificado** no nosso exemplo.) Adicione os seguintes grupos de campos ao schema:
...
Ao criar o fluxo de dados da conexão de origem para **Conjunto de relatórios A**, selecione **Esquema unificado** para uso no fluxo de dados. Adicione mapeamentos personalizados da seguinte maneira:
...
Agora crie um **Todos os Conjuntos de relatórios** conexão para CJA, combinando o conjunto de dados A e o conjunto de dados B.

Crie um **Exibição global** visualização de dados no CJA. Ignore os campos de eVar originais e inclua apenas os campos do grupo de campos Campos unificados.
...
Essencialmente, agora você mapeou o eVar 1 e o eVar 2 dos conjuntos de relatórios de origem para três novos campos. Observe que outra vantagem de usar os mapeamentos de Preparação de Dados é que os campos de destino agora são baseados em nomes semanticamente significativos (termo de pesquisa, unidade de negócios, categoria de cliente) em vez dos nomes de eVar menos significativos (eVar1, eVar2).

Observação: O grupo de campos personalizados Campos unificados e os mapeamentos de campo associados podem ser adicionados aos fluxos de dados e conjuntos de dados existentes do Conector de origem do Analytics a qualquer momento, no entanto, isso afetará somente os dados subsequentes.
...
Crie um schema no AEP. Adicione os seguintes grupos de campos ao schema:
...
Ao criar o fluxo de dados para **Conjunto De Dados A**, selecione **Esquema estendido** como esquema. Adicione mapeamentos personalizados da seguinte maneira:

| Conjunto de dados Um campo de origem | Campo Destino do grupo de campos Informações do cliente |
| --- | --- |
| \_experience.analytics.customDimensions.eVars.eVar2 | _\&lt;path>_.Customer_category |

Ao criar o fluxo de dados para **Conjunto de dados B**, selecione novamente **Esquema estendido** como esquema. Adicione mapeamentos personalizados da seguinte maneira:
...
A alteração da ID do componente é uma função somente CJA e não afeta dados do Conector de origem do Analytics enviados para o Perfil unificado e RTCDP.
