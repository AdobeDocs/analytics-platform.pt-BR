...
* Analise todos os fatores contextuais de experiências atraentes por meio de dados de características e comportamentais para melhorar os eventos de conversão.
* Use ferramentas avançadas de segmentação e ativação para perfis com pseudônimos que ainda não foram registrados ou convertidos.
* Dimensione experiências individuais em anúncios, redes sociais, email, texto e muito mais, usando interesses e canais preferidos para criar a personalização.
* Use a modelagem de IA para identificar indicadores e direcionar os usuários pelo melhor caminho, como avaliações gratuitas ou bate-papo.

A otimização de conversão do cliente tem tudo a ver com saldo. As métricas de sucesso de curto prazo (como a taxa de conversão) são importantes, mas as métricas de longo prazo (como fidelidade, valor vitalício, sentimento de marca e pontuação do promotor líquido) são o que garantem um futuro brilhante e sustentável para sua marca.

Portanto, em vez de tentar gerar o mesmo evento de conversão a partir de segmentos amplos, concentre-se em criar relações individualizadas com o cliente e analise os sinais e padrões de sucesso. Suas taxas de conversão melhoram naturalmente como um subproduto de uma estratégia centrada na experiência do cliente.

### Objetivos, desafios e diferenciais

O caso de uso de otimização de conversão do cliente aborda vários desafios comuns ao tentar alcançar seus objetivos. A combinação de aplicativos Adobe, como Real-time Customer Data Platform, Journey Optimizer e Customer Journey Analytics, e seus recursos subjacentes diferencia-se na superação desses desafios.

| Objetivo | Desafio | Diferenciação |
|---| ---|---|
| **Criar um profundo <br/>compreensão do<br/>cada indivíduo<br/>jornada do cliente potencial<br/>&amp; experiência.** | A maioria das organizações enfrenta o desafio de entender a conversão de nível macro (eventos geradores de receita). E, em seguida, emparelhe essa conversão de nível macro com os fatores que contribuem para qualquer tipo de evento de conversão.<br/><br/>A otimização de conversão também se limita aos canais digitais isoladamente (por exemplo, web ou celular). | O Adobe fornece um conjunto de aplicativos mais holístico e centrado no cliente, que permite que as marcas analisem todos os fatores contextuais que contribuem para experiências positivas e atraentes. E, em seguida, para melhorar significativamente os eventos de conversão, que são importantes para cada cliente em todos os canais.<br/><br/>Os insights podem ser acionados em toda a jornada para otimizar as experiências usando dados de características e comportamentais |
| **Identificar e analisar<br/>sinais e intenção<br/>para cada cliente potencial<br/>necessidades de conversão.** | É difícil para clientes de micro e nanosegmentos lançar campanhas direcionadas para diferentes coortes para impulsionar o registro ou a conversão<br/><br/>As marcas se esforçam para usar comportamentos anônimos a fim de planejar experiências impactantes em todos os canais. | O Adobe permite recursos avançados de segmentação e ativação para perfis com pseudônimos que ainda não foram registrados ou convertidos.<br/><br/>À medida que um usuário avança pelo processo de consideração para a conversão, os aplicativos Adobe podem trabalhar em conjunto para criar experiências omnicanal conectadas em publicidade, redes sociais, email, texto e muito mais. |
| **Envolver e<br/>personalizar para cada<br/>cliente potencial em conformidade<br/>em escala.** | A falta de conhecimento das interações do cliente em outros canais leva a mensagens irrelevantes ou obsoletas e corre o risco de causar danos econômicos a longo prazo à marca.<br/><br/>Difícil manter uma experiência coesa e conectada à medida que um usuário avança pelas etapas de consideração para conversão | Comece os clientes em sua jornada com sua marca facilitando o marketing personalizado e omnicanal que reconhece os interesses deles e os envolve nos canais de sua preferência<br/><br/>Use a modelagem de IA para identificar indicadores de forma inteligente e direcionar cada usuário para o melhor caminho, como um download de avaliação ou um chat. |
...
A otimização de conversão do cliente, como com qualquer outro caso de uso de insights e engajamento personalizado, requer uma arquitetura para dar suporte ao caso de uso. Consulte as visões gerais de arquitetura para:
...
* [Experience Platform e aplicativos](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/platform-applications)
* [Fluxo de dados Experience Platform](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/platform-data-flow)
* e sua implantação usando o [Experience Platform Web / SDK móvel e Edge Network](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/deployment/websdk) e a variável [SDKs do aplicativo](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/deployment/appsdk)

## Soluções e recursos

Para realizar e implementar casos de uso específicos de otimização de conversão do cliente, considere o uso dos seguintes aplicativos Adobe.
...
* [Análise de dados, inteligência e blueprints de IA/ML](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/data-exploration/overview), mais especificamente
   * [Análise de dados e blueprint de inteligência](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/data-exploration/analysis) e
   * [Ciência de dados personalizada para blueprint de enriquecimento de perfil](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/data-exploration/data-science).
...
| Use a oferta perfeita na hora certa. | Em algum momento do processo de consideração, pode ser necessário apenas uma boa oferta de uma marca para fechar o negócio. O Adobe Journey Optimizer facilita a criação e o fornecimento de ofertas altamente personalizadas que convertem navegadores em compradores. | <ul><li>[Sobre o Gerenciamento de decisão](https://experienceleague.adobe.com/pt-br/docs/journey-optimizer/using/decisioning/offer-decisioning/get-started-decision/starting-offer-decisioning)</li></ul> |
| Otimização do ciclo de vida do cliente | O Journey Optimizer oferece o que você precisa para se envolver, manter-se envolvido e tomar medidas para reengajar ativamente quando necessário. Você deseja que a conversão seja apenas uma das muitas que estão por vir. Com o Journey Optimizer, você tem acesso a todos os insights necessários para entender seus clientes e o que eles precisam. | <ul><li>[Relatórios ao vivo](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/live-report/live-report)</li><li>[Relatórios globais](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/global-report/global-report)</li><li>[Relatórios de canal](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/channel-report/channel-report-gs)</li><li>[Jornada relatórios](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/reports/sharing-overview)</li><li>[Customer Journey Analytics](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/cja-ajo) </li></ul> |
...
O exemplo abaixo ilustra o caso de uso de otimização de conversão do cliente para uma marca fictícia da Luma e um cliente fictício chamado Sarah. O exemplo fornece uma sequência de etapas. Para cada etapa são fornecidas uma descrição, o uso do aplicativo e instruções detalhadas.

| Etapa | Descrição | Uso de aplicativos | Instruções detalhadas |
|---|---|---|---| 
| 1 | Sarah é uma integrante da Luma plus que fez compras online e na loja. Recentemente, Sarah não viu nenhum produto de interesse e não fez nenhuma compra. | | |
| 2 | Você quer identificar um cliente como Sarah. Clientes que compraram anteriormente vestuário de ioga, mas não fizeram uma compra recente. | Você usa o Customer Journey Analytics para definir um filtro (segmento) de clientes que compraram produtos de ioga, mas sem uma compra recente.<br/>Aplique o filtro a uma tabela de forma livre da Campanha de marketing para identificar quais campanhas estão gerando mais vendas para esse grupo. | <ul><li>[Visão geral dos Filtros](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-components/cja-filters/filters-overview)</li><li>[Construtor de filtros](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-components/cja-filters/filter-builder)</li><li>[Filtrar e classificar tabelas de forma livre](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/visualizations/freeform-table/filter-and-sort)</ul> |
| 3 | Você também deseja vender para novos visitantes e prospetos | Use a IA do cliente para executar uma pontuação de propensão usando o modelo de venda adicional do cliente para clientes que provavelmente comprarão vestuário de ioga e criarão um segmento. <br/>No Real-time Customer Data Platform, você revisa o segmento e suas opções. <br/>Ative o segmento para o Target. | <ul><li>[Introdução à IA do cliente](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/visualizations/freeform-table/filter-and-sort)</li><li>[Criar segmentos de clientes com pontuações previstas](https://experienceleague.adobe.com/en/docs/experience-platform/intelligent-services/customer-ai/user-guide/create-segment)</li><li>[Conexão com o Adobe Target](https://experienceleague.adobe.com/en/docs/experience-platform/destinations/catalog/personalization/adobe-target-connection)</li></ul> |
| 4 | Sara está lendo as notícias da manhã e vê um anúncio de ioga da Luma. Sarah aceita cookies, clica no anúncio e notou que o site é personalizado para Sarah (usando Target). Sarah começa a navegar pelo conteúdo e o conteúdo é progressivamente personalizado com cada clique. | Use o Target para criar atividades que personalizam a experiência do visitante. | <ul><li>[Entender os tipos de atividade do Adobe Target](https://experienceleague.adobe.com/en/docs/target-learn/tutorials/activities/understanding-the-types-of-activities)</li><li>[Criar atividades de direcionamento de experiência](https://experienceleague.adobe.com/en/docs/target-learn/tutorials/activities/create-experience-targeting-activities)</li></ul> |
| 5 | Sarah adiciona os shorts de ioga ao seu carrinho de compras... e se inscreve usando um endereço de e-mail. | Você usa o Real-time Customer Data Platform para reconhecer que Sarah mudou de um perfil anônimo para um perfil conhecido. | <ul><li>[Procurar perfis no Real-time Customer Data Platform](https://experienceleague.adobe.com/en/docs/experience-platform/rtcdp/profile/profile-browse)</li><li>[Visualizador de gráficos de identidade](https://experienceleague.adobe.com/en/docs/experience-platform/identity/features/identity-graph-viewer)</li></ul> |
| 6 | Não tenho certeza sobre o produto, seu alto preço e não encontrar as informações que a Sara precisa antes de comprar o produto, Sarah decide ligar para a central de atendimento para obter ajuda. O representante da central de atendimento dá a Sarah alguns detalhes adicionais, explicando que Sarah é elegível para uma Luma mais oferta para uma aula de yoga gratuita na loja Luma nas proximidades. | Você usa um aplicativo de call center personalizado, integrado ao Real-time Customer Data Platform (por meio da API do Servidor de borda), para encontrar todas as informações relevantes sobre Sarah enquanto conversa. <br/>Use a Gestão de decisões para configurar as decisões, disposições e ofertas. | <ul><li>[Visão geral da API do servidor Edge Network](https://experienceleague.adobe.com/en/docs/experience-platform/edge-network-server-api/overview)</li><li>[Sobre o Gerenciamento de decisão](https://experienceleague.adobe.com/pt-br/docs/journey-optimizer/using/decisioning/offer-decisioning/get-started-decision/starting-offer-decisioning)</li></ul> |
...
* Melhor direcionamento, resultando em mais conversões.
