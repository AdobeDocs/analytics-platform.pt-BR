...
Para visualizar e exportar logs de auditoria, você deve ter recebido a **[!UICONTROL Acesso aos logs de auditoria]** permissão de controle de acesso no console Adobe. Para saber como gerenciar permissões individuais para recursos do CJA, consulte [documentação de controle de acesso](/help/getting-started/cja-access-control.md).

## Exibir o log de auditoria na interface do usuário
...
![registro de auditoria](assets/audit_ui.png)

Você pode selecionar quais colunas estão visíveis ao acessar o seletor de colunas na parte superior direita.

## Exibir informações sobre entradas de log individuais

Clique duas vezes no botão info (i) ao lado de uma descrição.

![registro de auditoria](assets/info-button-audit.png)

Os seguintes itens são mostrados:

| Item | Descrição |
| --- | --- |
| Nome da ação | Esta é a lista de ações possíveis: <ul><li>API_Request</li><li>Aprovar</li><li>Criar</li><li>Editar</li><li>Exportar</li><li>Login_failed</li><li>Login_success</li><li>Logout</li><li>Org_change</li><li>Atualizar</li><li>Compartilhar</li><li>Transferir</li><li>Cancelar aprovação</li><li>Não Compartilhar</li></ul> |
| Descrição | Um resumo da ação, do tipo de componente (com ID) e de outros valores. |
| Nome do usuário | O usuário que está tomando a ação. |
| Tipo de componente | Os possíveis tipos de componentes incluem: <ul><li>Anotação</li><li>Público-alvo</li><li>Métrica calculada</li><li>Conexão</li><li>Grupo_de_dados</li><li>Exibição_de_dados</li><li>Acesso_Recurso</li><li>Filtro</li><li>IMS_Org</li><li>Publicação de conteúdo para dispositivos móveis</li><li>Projeto</li><li>Relatório</li><li>Scheduled_Project</li><li>Usuário</li><li>User_Group</li></ul> |
| ID organizacional IMS | Uma ID exclusiva fornecida para sua instância quando você faz logon pela primeira vez no Adobe Experience Cloud. Ele deve estar no formato: xxx@AdobeOrg. |
| ID de usuário | Uma ID exclusiva que identifica o usuário que tomou esta ação. |
| Data da criação | Quando esta ação foi tomada. |
...
Selecione o ícone de funil (![filter](assets/filter-icon.png)) para exibir uma lista de controles de filtro para ajudar a limitar os resultados. Somente os últimos 1.000 registros são exibidos, independentemente dos vários filtros selecionados.
...
| [!UICONTROL ID de usuário] | Filtrar em um usuário específico pela ID do usuário. A ID de usuário pode ser encontrada selecionando o botão info (i) ao lado de um nome de usuário. |
| [!UICONTROL Email] | Filtre o endereço de email de um usuário específico. O email pode ser encontrado selecionando o botão info (i) ao lado de um nome de usuário. |
| [!UICONTROL ID de componente] | Filtrar em uma ID de componente específica. A ID de usuário pode ser encontrada selecionando o botão de informações (i) de um componente desejado. |
...
A tabela a seguir descreve quais ações em quais tipos de componentes são registradas por logs de auditoria:
...
| [!UICONTROL Org. IMS] | <ul><li>API_Request</li><li>Criar</li><li>Excluir</li><li>Editar</li></ul> |
...
Todas as ações que você pode executar na interface do usuário também podem ser realizadas usando chamadas de API. Consulte a [Documento de referência da API CJA](https://developer.adobe.com/cja-apis/docs/api/#tag/Audit-Logs) para obter mais informações.