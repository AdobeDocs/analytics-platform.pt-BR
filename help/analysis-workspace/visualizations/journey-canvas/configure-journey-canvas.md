...
Veja [a visão geral](/help/analysis-workspace/visualizations/journey-canvas/journey-canvas.md) da tela jornada para saber mais sobre a tela Jornada, incluindo:
...
1. Especifique as seguintes informações básicas para configurar a tela de Jornada:
   ...
   | Campo | Função |
   |---------|----------|
   | [!UICONTROL **Métrica primária**] | Determina as métrica usadas ao calcular a porcentagem e os valores em cada nó na jornada.<p>**Observação**: a escopo dos dados incluídos em cada percentual e valor de número é determinada pelo métrica escolhido na **[!UICONTROL tela jornada container]** campo. Por exemplo, se **[!UICONTROL a Pessoa]** for definida como o container, as estatísticas mostradas na jornada abrangem várias sessões para uma determinada pessoa. Se **[!UICONTROL Sessão]** for definida como o container, então as estatísticas mostradas na jornada serão restritas a uma única sessão definida para uma determinada pessoa.</p><p>Considere os seguintes exemplos de como a métrica primária afeta a porcentagem e os valores de número de cada nó:</p><ul><li>Se _As Pessoas_ são a métrica principal e _Pessoa_ é o container, então apenas as pessoas que têm uma evento que corresponde aos critérios de cada nó sucessivo na jornada se movem ao longo da jornada. A precipitação ocorre em uma nó quando uma pessoa nunca chegou a nenhum dos próximos nós imediatos na viagem. Eles podem ter realizado outras ações no site, mas não atenderam aos critérios definidos por qualquer um dos nós que seguir imediatamente.</li><li>Se _Pessoas_ é a principal métrica e _Sessão_ é o container, então apenas as pessoas que têm uma evento que corresponde aos critérios de cada nó na jornada dentro de uma única sessão se movem ao longo da jornada. O fallout ocorre em uma nó quando uma pessoa nunca chegou a nenhum dos próximos nós imediatos na jornada em uma única sessão. Eles podem ter realizado outras ações no site dentro da sessão, mas não atenderam aos critérios definidos por qualquer um dos nós que seguir imediatamente.</li></ul> <p>A métrica principal afeta os seguintes aspectos da visualização da tela da jornada:</p><ul><li>O número total exibido em cada nó.  <p>Por exemplo, se Eventos é o métrica principal, cada nó mostra o número de pessoas que tiveram uma evento que corresponde aos critérios desse nó (e cada nó anterior que o levou à jornada).</p></li><li>A porcentagem exibida em cada nó. (Depois que a visualização for criada, você poderá usar a variável **[!UICONTROL Menu suspenso do valor]** percentual para optar por mostrar a porcentagem do total, a porcentagem do nó anterior ou a porcentagem do nó inicial).)<p>Por exemplo, se Eventos é o métrica principal, cada nó mostra a porcentagem de pessoas que tiveram uma evento que corresponde aos critérios desse nó (e cada nó anterior que o levou à jornada).</p></li><li>Quando um dimensão é adicionado à visualização, os três principais nós da visualização são adicionados, com base nas métrica primárias.</li></ul> |
   | [!UICONTROL **Métrica secundária**] | Determina o métrica secundário que é usado no cálculo da porcentagem e dos valores em cada nó na jornada. O métrica secundário é opcional. <p>**Observação**: a escopo dos dados incluídos em cada percentual e valor de número é determinada pelo métrica escolhido na **[!UICONTROL tela jornada container]** campo. Por exemplo, se **[!UICONTROL a Pessoa]** for definida como o container, as estatísticas mostradas na jornada abrangem várias sessões para uma determinada pessoa. Se **[!UICONTROL Sessão]** for definida como o container, então as estatísticas mostradas na jornada serão restritas a uma única sessão definida para uma determinada pessoa.</p><p>Quando um métrica secundário é configurado, afeta os seguintes aspectos da visualização da tela da jornada:</p><ul><li>O número total mostrado em cada nó abaixo das métrica primárias. <p>Por exemplo, se Contas for o métrica secundário, o número de contas é mostrado no nó para todas as pessoas que alcançaram esse nó na jornada.</p></li><li>A porcentagem mostrada em cada nó abaixo da métrica primária. (Depois que a visualização é criada, você pode optar por mostrar a porcentagem do total ou do nó inicial.)</li><p>Por exemplo, se Sessões for a métrica secundária, cada nó mostrará a porcentagem de sessões que atingiram esse nó na jornada (a porcentagem do total ou do nó inicial).</p></li></ul> |
...
1. Selecione [!UICONTROL **Criar**].
   ...
   Se você selecionou uma jornada do Journey Optimizer, a jornada é exibida com as mesmas solicitar, sequência e estrutura que tem no Journey Optimizer. (Somente usuários com acesso ao Jornada otimizer podem selecionar uma jornada do Journey Optimizer.)
...
1. Defina qualquer uma das seguintes configurações exibidas na parte superior da visualização:
   ...
   | Configuração | Função |
   |---------|----------|
   | [!UICONTROL **Valor percentual**] | O valor percentual mostrado em cada nó da jornada.<p>![valor percentual](assets/journey-canvas-percentage.png)</p> <p>Considere o seguinte ao configurar os valores de porcentagem mostrados em nós na jornada:</p><ul><li>Uma porcentagem é mostrada em cada nó da métrica primária. Uma porcentagem também é mostrada para o métrica secundário se uma estiver configurada. (Para obter mais informações sobre as configurações de métrica primária e secundária, consulte [Comece a criar uma visualização](#begin-building-a-journey-canvas-visualization) da tela da jornada.)</li><li>As porcentagens incluem todas as pessoas ou sessões incluídas nos visualização de dados no intervalo de datas do painel. Se _pessoas_ ou _sessões são usadas_ depende da configuração container. (Para obter mais informações sobre a configuração de container, consulte [Comece a criar uma visualização](#begin-building-a-journey-canvas-visualization) da tela da jornada.)</li></ul> <p>Escolha entre as seguintes opções:</p> <ul><li>[!UICONTROL **Porcentagem de start nó**]: calcula as porcentagens mostradas em cada nó em relação ao start nó. As porcentagens são baseadas nas métrica primárias e secundárias selecionadas. <p>Uma _start nó_ é uma nó sem nós conectados que a precedem.</p><p>Uma jornada pode conter vários nós start. No entanto, [!UICONTROL **a porcentagem do total**] é usada se a jornada contiver dois ou mais nós start que cliente potencial a um nó comum. Se você quiser usar [!UICONTROL **Porcentagem de start nó**], atualize a jornada para que cada nó na jornada possa ser rastreada até um único start nó.</p></li><li>[!UICONTROL **Porcentagem das nó**] anteriores: calcula as porcentagens mostradas em cada nó em relação às nó anteriores. As porcentagens são baseadas nas métrica primárias e secundárias selecionadas.</li><li>[!UICONTROL **Porcentagem do total**]: calcula as porcentagens mostradas em cada nó em relação a todos os dados nos visualização de dados. As porcentagens são baseadas nas métrica primárias e secundárias selecionadas.</li></ul> |
   | [!UICONTROL **Configurações de seta**] | As setas que aparecem entre nós na tela Jornada podem ser configuradas para mostrar rótulos e valores personalizados. <p>![configurações de seta](assets/journey-canvas-arrow-settings.png)</p><p>_Rótulos_ são nomes personalizados que aparecem nas setas. Somente uma única etiqueta é exibida em uma determinada seta. Os rótulos podem ser qualquer um dos seguintes e são mostrados nesta ordem de preferência:</p><ol><li>Um nome personalizado foi adicionado da tela de Jornada (conforme descrito em [Adicionar ou atualizar um rótulo em uma seta](#add-or-update-a-label-on-an-arrow))</li><li>Um rótulo do Journey Optimizer</li><li>Uma condição do Journey Optimizer</li></ol><p>_Valores_ são os números e porcentagens que aparecem nas setas e indicam as pessoas ou sessões que passaram de um nó para outro na jornada. (Em outras palavras, aqueles que não caíram da jornada em uma determinada etapa.) </p><p>As seguintes opções estão disponíveis para jornadas que não se originaram do Journey Optimizer e para jornadas do Journey Optimizer que não foram modificadas significativamente na tela do Jornada: (Modificações significativas incluem adicionar ou remover nós, adicionar ou remover setas ou alterar os componentes de um nó.)</p><ul><li>[!UICONTROL **Nenhum rótulo**]: nenhum rótulo é mostrado nas setas da jornada. </br> Essa opção só estará disponível se a jornada tiver sido modificada em </li><li>[!UICONTROL **Somente rótulos**]: os rótulos são mostrados nas setas da jornada.</li></ul><p>As seguintes opções estão disponíveis para o Journey Optimizer jornada que foram significativamente modificadas na tela do Jornada: (Modificações significativas incluem adicionar ou remover nós, adicionar ou remover setas ou alterar os componentes de um nó.)(**Observação**: essas opções são exibidas somente quando os dados do Journey Optimizer são detectados na mesma visualização de dados selecionada no painel do Analysis Workspace ao qual você está adicionando a visualização. Para obter informações sobre como alterar a visualização de dados em um painel no Analysis Workspace, consulte [visão geral do Analysis Workspace](/help/analysis-workspace/home.md).)</p><ul><li>[!UICONTROL **Nenhum rótulo ou valor**]: nenhum rótulo ou valor é mostrado nas setas da jornada.</li><li>[!UICONTROL **Somente rótulos**]: somente rótulos são exibidos nas setas da jornada. Os valores não são exibidos.</li><li>[!UICONTROL **Somente valores**]: somente valores são mostrados nas setas da jornada. Os rótulos não são exibidos.</li><li>[!UICONTROL **Valores e rótulos**]: ambos os rótulos e valores são mostrados nas setas da jornada.</li></ul> |
...
1. Arraste métricas, dimensões, itens de dimensão, filtros ou intervalos de datas do painel esquerdo para a tela. Métricas baseadas em um [campo derivado](/help/data-views/derived-fields/derived-fields.md) têm suporte. No entanto, métricas calculadas, bem como qualquer métrica ou dimensão que seja baseada em um [conjunto de dados de resumo](/help/data-views/summary-data.md), não são suportadas.
   ...
   | Tipo de componente | Posicionamento do componente | Atualizações de visualização após a adição de nó |
   |---------|----------|----------|
   | Métrica | Área em branco da tela | A nó exibe onde o componente foi descartado, não conectado a quaisquer nós existentes. |
   ...
   | Métrica | Uma seta entre 2 nós existentes | O nó é exibido entre os dois nós existentes em que o componente foi descartado e está conectado a ambos os nós existentes. (Consulte [Nós de](#connect-nodes) conexão para obter mais informações.)</p> |
...
#### Exibir os principais nós após um nó existente
...
1. Clique com o botão direito na seta entre os dois nós em que deseja mostrar os itens dimensão superiores.

1. Selecione [!UICONTROL **Exibir principais nós entre esses nós**].
...
Os nós são conectados como um &quot;caminho eventual&quot;, o que significa que os visitantes são contados desde que eventualmente se movam de um nó para outro, independentemente de qualquer evento que ocorra entre os dois nós. O tempo alocado para os usuários se moverem ao longo do caminho é determinado pela configuração container. <!-- It can also be controlled by [adding a time constraint](#add-a-time-constraint-between-nodes). -->

Você pode visualização a lógica dos nós conectados clicando com o botão direito do mouse na nó e selecionando [!UICONTROL **Criar filtro do nó**]. A lógica é mostrada na [!UICONTROL **seção Definição**] .
...
1. Selecione a dimensão que deseja usar para a detalhamento.

   Se você escolheu visualização o detalhamento na tela Jornada, os 5 itens principais dimensão serão mostrados no nó. Uma opção está disponível no nó para abrir a detalhamento em uma tabela de forma livre.
