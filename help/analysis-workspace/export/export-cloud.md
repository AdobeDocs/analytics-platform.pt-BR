...
>abstract="Exportações de tabelas completas estarão disponíveis assim que você visualizar os dados no Analysis Workspace. É possível criar ou agendar exportações de tabelas completas, conforme a necessidade.<br><br>É possível criar ou agendar exportações de tabela completas em apenas alguns minutos caso já saiba quais dados serão incluídos na exportação."
...
>title="Notificações"
>abstract="Adicione usuários e grupos para receberem notificações quando a exportação falhar ou estiver prestes a expirar."
...
* Métricas calculadas que usam funções sem suporte na definição de métrica (consulte [Funções de métrica calculada sem suporte](#calculated-metric-functions-support) para obter mais informações)
...
| Média cumulativa | ![AzulStatus](/help/assets/icons/StatusBlue.svg) Planejado |
