...
1. Na caixa de diálogo [!UICONTROL **Nova exportação de tabela completa**], especifique as seguintes informações:
   ...
   | Tags | É possível aplicar uma tag existente à exportação ou criar e aplicar uma nova tag. <p>Para aplicar uma tag existente à exportação, selecione qualquer tag no menu suspenso. Todas as tags da sua empresa estão disponíveis para aplicação<!-- double-check this -->.</p> <p>Para criar uma nova tag, digite o nome da nova tag e pressione Enter.</p><p>Considere o seguinte ao aplicar tags a uma exportação: <ul><li>As tags aplicadas podem ser filtradas ou pesquisadas na tabela de exportações.</li> <li>As tags aplicadas a um projeto não são aplicadas automaticamente ao exportar uma tabela completa, conforme descrito em “Configurar colunas na página de exportações”, em [Gerenciar exportações](/help/components/exports/manage-exports.md). (Alternativamente, ao [agendar um projeto completo para exportação](/help/analysis-workspace/curate-share/t-schedule-report.md), todas as tags aplicadas ao projeto serão automaticamente aplicadas à exportação.)  <!-- Right now we don't have a column for them on the exports table, so this isn't true. Jaden is adding the column. --></li></ul> |
...
## Requisitos e recursos não compatíveis {#export-requirements}

Esta seção detalha os requisitos mínimos e os recursos não compatíveis que você deve considerar ao exportar dados.
