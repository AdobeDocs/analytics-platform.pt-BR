...
3. Clique na guia Componentes à esquerda e arraste a dimensão &quot;ID do conjunto de dados&quot; para o local central chamado &quot;Dimension or Item&quot;.
...
A janela de pesquisa para rechaveamento depende da frequência desejada de [repetição](replay.md) de dados. Por exemplo, se você configurar o CCA para reproduzir dados uma vez por semana, a janela de retrospectiva para rechaveamento será de sete dias. Se você configurar o CCA para reproduzir dados todos os dias, a janela de pesquisa para rechaveamento será de um dia.
...
## Como o CCA trata situações em que uma única pessoa tem muitas IDs persistentes?
...
[Análise entre canais](../overview.md) é um recurso específico do CJA que permite entender como as pessoas operam em dispositivos e canais. É uma nova chave da ID pessoal de um conjunto de dados, permitindo que esse conjunto de dados seja combinado perfeitamente com outros conjuntos de dados. Esse recurso opera em design de forma semelhante à compilação em campo do CDA, mas a implementação é diferente para a arquitetura de dados diferente entre o Analytics tradicional e o CJA.
...
* Se o campo `Transient ID` estiver em branco, o CCA também deixará `Stitched ID` em branco. Nesse caso, `Persistent ID`, `Transient ID`e `Stitched ID` estão em branco no evento. Esses tipos de eventos são descartados de qualquer conexão do CJA usando o conjunto de dados que está sendo compilado em que `Stitched ID` foi escolhido como `Person ID`.
