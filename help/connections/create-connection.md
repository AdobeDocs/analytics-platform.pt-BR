...
Um novo workflow Connections está sendo iniciado no Customer Journey Analytics (CJA) em maio de 2022. Esta é uma visão geral dos novos recursos:
...
1. No CJA, clique na guia Conexões .
...
1. Agora, configure os conjuntos de dados um por um.
   ...
   | Carimbo de data e hora | Somente para conjuntos de dados de eventos, essa configuração é definida automaticamente para o campo de carimbo de data e hora padrão a partir de esquemas baseados em eventos no Experience Platform. |
   | Importar novos dados | Selecione essa opção se quiser estabelecer uma conexão contínua, para que qualquer novo lote de dados adicionado aos conjuntos de dados nesta conexão continuem automaticamente fluindo para o Espaço de trabalho. Pode ser definido como Ligado ou Desligado. |
   ...
   | Status de preenchimento retroativo | Os possíveis indicadores de status são:<ul><li>Sucesso</li><li>Processamento de X preenchimento retroativo</li><li>Desligado</li></ul> |
   | ID do conjunto de dados | Essa ID é gerada automaticamente. |
   | Descrição | A descrição fornecida para esse conjunto de dados quando ele foi criado. |
   | Tamanho do conjunto de dados | O tamanho do conjunto de dados. |
   | Esquema | Este é o esquema com base no qual o conjunto de dados foi criado no Adobe Experience Platform. |
   | Conjunto de dados | O nome do conjunto de dados. |
   | Visualização: `<dataset name>` | Pré-visualiza o conjunto de dados com colunas de data, ID e Identificador. |
   | Remover | Remova este conjunto de dados da conexão. |
