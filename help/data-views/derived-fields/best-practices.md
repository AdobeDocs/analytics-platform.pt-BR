...
As seções neste artigo são organizadas por tema. De cadeias de regras muito complexas ao uso incorreto de funções como [Pesquisa](./derived-fields.md#lookup), [Substituição de Regex](./derived-fields.md#regex-replace) e [Próximo ou Anterior](./derived-fields.md#next-or-previous). Cada seção inclui:
...
* Operações simples como [Letras minúsculas](./derived-fields.md#lowercase), [Aparar](./derived-fields.md#trim) ou [Letras maiúsculas e minúsculas quando](./derived-fields.md#case-when) as verificações de URL da página geralmente são mais caras do que a mesma lógica em campos de baixa cardinalidade, como nome da página, seção do site ou grupo de URLs.

### Diagnóstico de risco: desempenho
...
* Um campo derivado replica o comportamento já disponível nas configurações do componente:
   ...
   * Uma métrica derivada que simplesmente conta uma métrica existente sob uma condição.
     ...
     Isso replica o que uma métrica filtrada ou [Incluir valores de exclusão](/help/data-views/component-settings/include-exclude-values.md) poderia alcançar.
...
* [Substituição de Regex](./derived-fields.md#regex-replace) ou as condições baseadas em regex usam padrões muito amplos em que as funções [Caso Quando](./derived-fields.md#case-when) mais simples com **[!UICONTROL Contém]** ou **[!UICONTROL Começa com]** são uma alternativa mais simples e melhor.
...
>[!NOTE]
>
>Reduza a dependência em campos de alta cardinalidade dentro de campos derivados complexos sempre que possível (por exemplo: use chaves normalizadas ou classificações agrupadas) para limitar o custo da consulta e a probabilidade de atingir [limites de operador ou função](./derived-fields.md#limitations).

Máximo de funções e operadores por campo derivado de [documentos](./derived-fields.md#limitations) do Customer Journey Analytics, incluindo limites por tipo de função ([Pesquisa](./derived-fields.md#lookup), [Matemática de Data](./derived-fields.md#date-math), [Desduplicar](./derived-fields.md#deduplicate), [Matemática](./derived-fields.md#math), [Divisão](./derived-fields.md#split), [Análise de URL](./derived-fields.md#url-parse) e muito mais).
...
* Divida a lógica em vários campos derivados que são encadeados (por exemplo: um campo derivado A que normaliza uma chave de pesquisa e um campo derivado Band normalize_key then lookup_label).
