...
A maioria das dimensões e métricas no CJA são baseadas em elementos de esquema do conjunto de dados do Adobe Experience Platform. No entanto, vários componentes estão disponíveis para adicionar a uma visualização de dados, independentemente da conexão usada.

[!UICONTROL Componentes padrão] são componentes que não são gerados de campos de esquema do conjunto de dados, mas que são gerados pelo sistema. Alguns componentes do sistema são necessários para facilitar os recursos de relatórios no Analysis Workspace, enquanto outros componentes do sistema são opcionais.

![Componentes padrão](assets/standard-components.png)

## Componentes padrão obrigatórios

Esses componentes padrão obrigatórios são adicionados a cada visualização de dados por padrão. Eles são essenciais para os recursos de relatório que o Customer Journey Analytics oferece.

| Nome do componente | Dimensão ou métrica | Notas |
| --- | --- | --- |
| [!UICONTROL Pessoas] | Métrica | Com base na ID da pessoa especificada em [!UICONTROL Connection]. |
| [!UICONTROL Sessões] | Métrica | Com base nas configurações de sessão da visualização de dados. |
| [!UICONTROL Eventos] | Métrica | O número de linhas de todos os conjuntos de dados de eventos em uma [!UICONTROL Connection]. |
...
Os componentes padrão opcionais estão disponíveis na guia **[!UICONTROL Visualizações de dados]** > **[!UICONTROL Editar visualização de dados]** > **[!UICONTROL Componentes]** > guia **[!UICONTROL Componentes padrão]** .
...
| [!UICONTROL Tempo gasto por evento] | Dimensão | Coloca a métrica [!UICONTROL Tempo gasto] em compartimentos [!UICONTROL Evento]. |
| [!UICONTROL Tempo gasto por sessão] | Dimensão | Coloca a métrica [!UICONTROL Tempo gasto] em compartimentos [!UICONTROL Sessão]. |
| [!UICONTROL Tempo gasto por pessoa] | Dimensão | Coloca a métrica [!UICONTROL Tempo gasto] em compartimentos [!UICONTROL Pessoa]. |
