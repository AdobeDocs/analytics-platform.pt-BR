...
| Feeds de dados | A exportação de dados de primeira geração de conjuntos de dados está disponível por meio do [API de acesso a dados da AEP](https://experienceleague.adobe.com/docs/experience-platform/data-access/api.html?lang=en) e [Destinos da AEP](https://experienceleague.adobe.com/docs/experience-platform/destinations/ui/activate/export-datasets.html?lang=en). Essas opções fornecem exportação no nível da ocorrência/linha de todos os dados coletados ou assimilados no AEP Data Lake. As colunas de dados do pós-processo não estão disponíveis porque as colunas de publicação são calculadas no momento da consulta. A exportação de colunas de publicação está disponível por meio de relatórios. |
...
| Relatório de Data Warehouse | O suporte é planejado na interface do Analysis Workspace. O [[!UICONTROL Serviço de consulta]](https://experienceleague.adobe.com/docs/experience-platform/query/home.html?lang=pt-BR) da Adobe Experience Platform também fornece uma interface para esses casos de uso no CJA. |
...
A tabela a seguir lista os recursos que estão disponíveis no Customer Journey Analytics (CJA), mas que não são compatíveis com o Adobe Analytics (AA).

| Recurso | Mais detalhes |
| --- | --- |
| Alojamento de qualquer tipo de dados | O CJA é combinado com o Experience Platform para comportar todos os tipos de esquemas e tipos de dados. Usando [Experience Data Model (XDM)](https://experienceleague.adobe.com/docs/experience-platform/xdm/home.html?lang=pt-BR), os dados podem ser representados e organizados uniformemente, prontos para combinação e exploração. O Adobe Analytics é predominantemente voltado para dados de análise da Web e de dispositivos móveis com alguns recursos para [importar dados](https://experienceleague.adobe.com/docs/analytics/import/home.html?lang=pt-BR). |
| Dimension e métricas ilimitadas do cliente | As dimensões CJA são ilimitadas; podem ser numéricos, de texto, de objetos, de listas ou de misturas de todos. Dimension pode ser aninhado ou hierárquico. O Analytics suporta até 75 props e 250 eVars. Isso remove as limitações de medição atuais usando dimensões e eventos. |
| cardinalidade/valores exclusivos ilimitados | O CJA suporta valores exclusivos ilimitados ou itens de dimensão que podem ser reportados em uma única dimensão. AA é limitado a 500K valores únicos. Isso remove as limitações de relatórios e análises existentes atualmente com a implementação do Analytics em grande escala. |
| Transformações de tempo do relatório | As Transformações de tempo de relatório (mais conhecidas como Exibições de dados) no CJA permitem que você interprete os dados a partir de uma conexão. Você pode alterar ou remover dados sem reimplementar; usar subsequências para manipular dimensões; criar métricas a partir de qualquer valor; filtrar subeventos. E tudo isso pode ser feito de forma não destrutiva. O Adobe Analytics fornece recursos limitados por meio de conjuntos de relatórios virtuais e sessões. |
| Análise de experiência | O CJA pode avaliar o incentivo e a confiança de qualquer experimento a partir de qualquer fonte de dados definida como parte de uma conexão. Isso permite compreender os relacionamentos de causa e efeito entre as interações do cliente que abrangem qualquer canal. O Analytics está limitado a análises de experimentação por meio da integração do Analytics for Target (A4T). |
| Análise entre dispositivos | O CJA oferece suporte à combinação perfeita de conjuntos de dados específicos de dispositivo de sessões não autenticadas e autenticadas. Você também pode preencher dados históricos em dispositivos conhecidos. No Analytics, esse recurso está limitado a um único conjunto de relatórios e ao uso de um gráfico de dispositivos. |
| Acesso SQL | Usando a opção Data Distiller, o CJA pode remover as limitações de dados coletados no processamento backend do Adobe. Você pode modificar seus dados com o SQL, criar novos valores e conjuntos de dados exclusivos para sua empresa e continuar a explorar. O Analytics não oferece suporte a nenhum tipo de acesso SQL a seus dados. |
| Opções de segurança e privacidade aprimoradas - prontidão para HIPAA | O CJA está pronto para HIPAA e oferece opções de segurança adicionais para conformidade com a regulamentação. O Adobe Analytics não está preparado para HIPAA. |
