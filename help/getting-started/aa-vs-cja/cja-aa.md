...
| Compilação entre dispositivos/canais | Compatível com conjuntos de dados que contenham informações de identidade diretas (também conhecido como compilação “baseada em campo”); a compilação baseada em gráfico ainda não é compatível, mas está planejada. Consulte [Cross-Channel Analytics](/help/connections/cca/overview.md). |
...
| A4T | O suporte parcial é fornecido por meio de campos no [Conector de origem do Analytics](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html). Há planejamento de suporte para nomes amigáveis do A4T em atividades de direcionamento e experiência. |
