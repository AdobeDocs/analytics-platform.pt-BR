...
| Relatório de aumento e confiança | Suporte completo via [Painel de experiência](/help/analysis-workspace/c-panels/experimentation.md) |
...
| Análise de mídia de transmissão | Os dados de mídia estão disponíveis usando o Conector de dados do Analytics como parte do painel Visualizadores simultâneos de mídia e do painel Tempo gasto na reprodução da mídia no Workspace. |

{style=&quot;table-layout:auto&quot;}

## Suportado de uma nova maneira

| Recurso | Notas |
| --- | --- |
| Publicação de público-alvo (Publicação de segmento) | Suportado se licenciado com produtos Adobe Data Platform ou Journey Optimizer. A [Publicação de público-alvo](/help/components/audiences/audiences-overview.md) envia públicos-alvo para o Perfil do cliente em tempo real na Experience Platform. |
...
| Criador de regras de classificação | Suportado com [subsequências](/help/data-views/component-settings/substring.md) no CJA. Usa manipulações de string de caracteres no momento do relatório em vez de conjuntos de dados de pesquisa. |
...
| Persistência da variável de merchandising | Suporte completo através de [dimensões de ligação e métricas de ligação](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/component-settings/persistence.html#binding-dimension) |
...
| Desduplicação de métrica | Agora configurado em métricas nas Visualizações de dados. A desduplicação de métricas ocorre no nível da pessoa ou da sessão, em vez do nível do Conjunto de dados, da Exibição de dados ou da Conexão. |
...
| Relatório de Sessão Nova vs. Repetida | Realizado anteriormente usando a dimensão Número de visitas . Sessões novas versus repetidas são compatíveis [com uma janela de retrospectiva de 13 meses](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/data-views-usecases.html?lang=pt-BR#new-repeat). |
...
| Report Builder (plug-in do Excel) | Suportado com um novo plug-in do Office 365 para Excel. |
| Permissões do usuário/Controles de acesso de dados | O CJA distingue entre [Adobe Admin Console](https://experienceleague.adobe.com/docs/core-services/interface/administration/admin-getting-started.html?lang=pt-BR) administradores de produto, administradores de perfil de produto e usuários do . Somente administradores de produtos podem Criar/atualizar/excluir conexões, projetos, filtros ou métricas calculadas que foram criadas por outros usuários, enquanto administradores de produtos e administradores de perfis de produtos podem editar Exibições de dados. Permissões de usuário adicionais estão disponíveis para coisas como criar métricas calculadas, filtrar ou anotações. |
| Regras de processamento, regras VISTA e regras de processamento de canais de marketing | Suportado com a funcionalidade Prep de dados do Adobe Experience Platform para conjuntos de dados baseados em SDK da Web, bem como dados do Conector de dados do Analytics. |
...
| Recurso | Notas |
| --- | --- |
| Canais de marketing | Os dados de Canais de marketing agora fluem para o CJA por meio do Conector de origem do Analytics. As regras de Canal de marketing ainda devem ser configuradas no Adobe Analytics tradicional e algumas regras não são compatíveis. Para obter mais detalhes, consulte a [documentação dos Canais de marketing do CJA](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-usecases/marketing-channels.html?lang=pt-BR#cja-usecases). Além disso, para implementações do WebSDK, os plug-ins estão disponíveis para definir canais de marketing no lado do cliente. O suporte futuro para regras de processamento de canais de marketing em tempo de relatório está planejado. |
| Compilação entre dispositivos/canais | Suportado para conjuntos de dados que contenham informações de identidade diretamente (também conhecido como compilação &quot;em campo&quot;); A compilação em gráfico ainda não é suportada, mas planejada. Consulte [Cross-Channel Analytics](/help/connections/cca/overview.md). |
...
| A4T | O suporte parcial é fornecido por meio de campos no [Conector de origem do Analytics](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html). Há suporte para nomes amigáveis do A4T em atividades e experiências do Target. |
...
| Feeds de dados | O suporte é planejado por meio de destinos AEP. |
