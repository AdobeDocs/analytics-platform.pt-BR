...
* **Mapeamento campos do `eVars` para nomes semanticamente significativos**. `eVars` e `props` provenientes do conector de origem do Analytics são mapeados para campos como _\_experience.analytics.customDimensions.eVars.eVar 1_. O preparo de dados pode ser usado para mapear campos do `eVar` e do `prop` para novos campos que tenham nomes mais significativos para seus usuários ou que correspondam a nomes provenientes de outras fontes de dados. (Isso também pode ser feito por outros meios, como renomear os campos em uma [Visualização de dados do Customer Journey Analytics](/help/data-views/create-dataview.md).)
* **Dados de transformação geral**. O Preparo de dados tem centenas de funções de mapeamento que podem ser usadas para calcular e calcular novos campos com base nos dados que vêm pelo conector de origem do Analytics. Você pode dividir campos delimitados em campos separados. É possível combinar campos. Você pode manipular strings. Você pode extrair informações de um campo com base em expressões regulares e muito mais.
