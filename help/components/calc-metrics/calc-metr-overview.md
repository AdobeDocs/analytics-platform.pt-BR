...
* Create filtered metrics that are derived at report run time, [without having to change the implementation](https://youtu.be/CuQTm9RaUpY). Eles podem ser vistos historicamente porque são baseados em filtros.
...
* (Somente métricas calculadas avançadas) Filtrar em métricas. Por exemplo, é possível criar uma métrica para “Novos visitantes”, com uma contagem de pessoas para as quais esta é a primeira sessão.
