...
O Customer Journey Analytics fornece várias métricas calculadas para abranger os casos de uso mais comuns. Essas métricas calculadas estão disponíveis por padrão no Analysis Workspace.
...
>[!NOTE]
>
>Além das métricas calculadas padrão descritas nesta página, também é possível adicionar métricas calculadas adicionais a uma visualização de dados.
>
>É possível:
...
|---------|----------|---------|
| Taxa de rejeição | A proporção de visitas que continham exatamente uma ocorrência em comparação ao número de visitas nessa página. Isso pode ajudar a entender quais itens de dimensão têm a maior taxa de rejeição, ou para ver uma taxa de rejeição total agregada do site ao longo do tempo. | `[Bounces] / [Entries]` |
...
| Usuários (Dispositivo móvel) | O número total de usuários de um aplicativo para dispositivos móveis | `[Mobile App Users] (filter)`<br>`[Unique Visitors] (metric)` |
...
| Velocidade da página | O número de exibições de página adicionais geradas por um conteúdo. Isso pode ajudar você a determinar qual conteúdo gera envolvimento adicional. | `[Page Views] / [Visits]` |
| Taxa de conversão | A porcentagem de visitantes que executaram uma ação desejada, como fazer uma compra. | `[Orders] / [Visits]` |
| Duração média da sessão (móvel) | O tempo médio que os visitantes passam no site durante uma única sessão. | Em branco |
...
| Visitantes únicos estimados (ITP 2.1) | Para visitantes ITP (usuários em navegadores Safari), divida Visitantes únicos por 2 ou menos. Essa métrica calculada assume que você está definindo cookies usando JavaScript do lado do cliente (não usando uma implementação CNAME). As implementações que definem cookies usando JavaScript do lado do cliente foram afetadas a partir da ITP 2.1. Consulte [Prevenção de rastreamento inteligente](https://webkit.org/blog/8613/intelligent-tracking-prevention-2-1/) para obter detalhes. | `[Unique Visitors (metric) with ITP Visitors (ITP 2.1, Non-CNAME implementations) filter] / [Unique Visitors metric + Non-ITP Visitors (ITP 2.1, Non-CNAME implementations) filter]` |
