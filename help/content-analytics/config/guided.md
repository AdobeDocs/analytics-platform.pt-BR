...
1. Opcionalmente. especifique a forma como o conteúdo é renderizado em seu site. Os parâmetros são zero ou mais combinações de uma **[!UICONTROL expressão regular de domínio]** e **[!UICONTROL parâmetros de consulta]**.
...
* Para criar uma nova propriedade de Tags:
  ...
  ![Nova Marca da Coleção de Dados de Análise de Conteúdo](../assets/aca-configuration-datacollection-newtag.png)
   ...
   2. Especifique um **[!UICONTROL Nome de marcas]**, por exemplo `ACA Test`.
...
## Configurações e definições de integração

As seções a seguir descrevem as definições e configurações aplicadas ao [Customer Journey Analytics](#customer-journey-analytics-cja), [Experience Platform](#experience-platform-aep) e [Coleção de Dados](#data-collection-dc) como parte da implementação de uma configuração do Content Analytics.
